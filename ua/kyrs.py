...
async def get_kurs(message: types.Message):
    ...
    try:
        ...
        await message.answer(hbold(f"---{datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}---\n"
                                   f"{message.text} гривень у різних валютах:\n"
                                   ...
                                   f"{message.text} ₴  = {round(float(message.text) / yun, 2)} ¥"), reply_markup=ua.f1.back)

    except ValueError:
        await message.answer("Забагато запитів,спробуйте трохи пізніше", reply_markup=ua.f1.back)
